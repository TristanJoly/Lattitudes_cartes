...
# fond blanc (optionnel pour lisibilité)
folium.Rectangle(bounds, color="#ffffff", fill=True, fill_opacity=1).add_to(m)
...
if clicked:
    ...
    if st.session_state.get("selected_dep") != clicked_norm:
        ...
        try:
            ...
            # fallback : modifier les query params pour forcer un rerun
            try:
                params = st.experimental_get_query_params()
            except Exception:
                params = {}
            params["_rerun"] = str(int(time.time() * 1000))
            st.experimental_get_query_params(**params)
