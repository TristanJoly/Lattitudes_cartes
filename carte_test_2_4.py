...
"""
    st.markdown("## 🩺 Diagnostic flash")

    if sel_row is not None:

        # ---------- 1. CALCUL DES TRIGGERS (TW) ----------
        triggered_alerts = []

 

        # --- MÉTRIQUE : maladies / personnes ---
        if (
            sel_row.get("access_med_generalistes", 0) < national_means.get("access_med_generalistes", 0)
            and sel_row.get("hopital_court_sejour", 0) + sel_row.get("services_urgence", 0) > national_means.get("hopital_court_sejour", 0) + national_means.get("services_urgence", 0)
        ):
            triggered_alerts.append((
                "Désertification Médicale Critique",
                "Déploiement de cabines de téléconsultation assistée"
            ))

        if (
            sel_row.get("soins_domicile_personnes_agees", 0) + sel_row.get("aide_menagere_personnes_agees", 0) < 
            national_means.get("soins_domicile_personnes_agees", 0) + national_means.get("aide_menagere_personnes_agees", 0)
            and sel_row.get("dont part des 75 ans ou plus (en %)", 0) > national_means.get("dont part des 75 ans ou plus (en %)", 0)
        ):
            triggered_alerts.append((
                "Risque de Rupture de Maintien à Domicile",
                "agir sur le nombre d’aides à domicile"
            ))

        if (
            sel_row.get("75_plus_isoles", 0) > national_means.get("75_plus_isoles", 0)
            and sel_row.get("75_plus_sans_voiture", 0) > national_means.get("75_plus_sans_voiture", 0)
        ):
            triggered_alerts.append((
                "Fragilité Préventive",
                "augmenter les campagnes de prévention"
            ))

        # --- MÉTRIQUE : pauvreté ---
        if (
            sel_row.get("Taux de pauvrete pour plus de 75 ans", 0) > national_means.get("Taux de pauvrete pour plus de 75 ans", 0)
            and sel_row.get("60_75_plus_proprietaires", 0) < national_means.get("60_75_plus_proprietaires", 0)
        ):
            triggered_alerts.append((
                "Précarité Locative",
                "Renforcement du Fonds de Solidarité Logement Seniors"
            ))

        if (
            sel_row.get("APL_SAPA", 0) < national_means.get("APL_SAPA", 0)
            and sel_row.get("Taux de pauvrete pour plus de 75 ans", 0) > national_means.get("Taux de pauvrete pour plus de 75 ans", 0)
        ):
            triggered_alerts.append((
                "Risque de Renoncement aux Soins",
                "agir sur l’accessibilité (transport / remboursement trajets)"
            ))
        tw_count = len(triggered_alerts)

    # --- MÉTRIQUE : isolement ---
    if (
        sel_row.get("60_75_plus_isoles", 0) > national_means.get("60_75_plus_isoles", 0)
        and sel_row.get("60_75_plus_sans_voiture", 0) > national_means.get("60_75_plus_sans_voiture", 0)
    ):
        triggered_alerts.append((
            "Enclavement Sanitaire",
            "déploiement de navettes communales"
        ))

        tw_count = len(triggered_alerts)

        # ---------- 2. COULEUR DU DIAGNOSTIC ----------
        if tw_count == 0:
            diag_color = "#7FB069"   # vert
        elif tw_count <= 2:
            diag_color = "#F4D35E"   # jaune
        elif tw_count <= 4:
            diag_color = "#F7A072"   # orange
        else:
            diag_color = "#EE6352"   # rouge

        # ---------- 3. AFFICHAGE ----------
        left_diag, right_diag = st.columns([1.4, 1])
        ...
        with left_diag:
            st.markdown(
                ...
                <div style="
                    ...
                    border-radius:14px;
                ">
                <b>{tw_count} signaux de vulnérabilité détectés</b>
                </div>
                """,
                unsafe_allow_html=True
            )
            ...
            if triggered_alerts:
                for title, action in triggered_alerts:
                    st.markdown(f"""
                    **{title}**  
                    → *{action}*
                    """)
            else:
                st.markdown("Aucun signal critique détecté pour ce département.")

        # ---------- 4. SPIDER GRAPH VULNÉRABILITÉS ----------
        with right_diag:
            vuln_labels = ["Sanitaire", "Économique", "Social"]

            vuln_dep = [
                sel_row["indice_sanitaire"],
                sel_row["indice_economique"],
                sel_row["indice_social"]
            ]

            vuln_nat = [
                df["indice_sanitaire"].mean(),
                df["indice_economique"].mean(),
                df["indice_social"].mean()
            ]
        ...
        st.info("Sélectionnez un département pour afficher le diagnostic.")
